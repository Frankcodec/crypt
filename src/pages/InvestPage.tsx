...
const PaymentModal = ({ show, onClose, amount, plan, refreshData }: any) => {
  ...
  const handleConfirm = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost/crypto-backend/purchase_plan.php', {
        method: 'POST',
        mode: 'cors',
  ...
  const fetchData = async () => {
    try {
      const [pRes, mRes, bRes] = await Promise.all([
        fetch('http://localhost/crypto-backend/get_plans.php'),
        fetch(`http://localhost/crypto-backend/get_my_investments.php?user_id=${user.id}`),
