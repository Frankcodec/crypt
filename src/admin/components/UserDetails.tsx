...
const UserDetails: React.FC = () => {
  ...
  const fetchFullDetails = async () => {
    try {
      const res = await fetch(`http://localhost/crypto-backend/admin/get_user_full_details.php?id=${id}`);
  ...
  const handleManualDeposit = () => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch('http://localhost/crypto-backend/admin/manual_deposit.php', {
            method: 'POST',
