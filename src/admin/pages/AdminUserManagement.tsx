...
const AdminUserManagement: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      const res = await fetch('http://localhost/crypto-backend/admin/get_all_users.php');
  ...
  const handleAddBalance = (user: UserData) => {
    MySwal.fire({
      ...
      if (result.isConfirmed && result.value) {
        try {
          const res = await fetch('http://localhost/crypto-backend/admin/update_user_balance.php', {
            method: 'POST',
