...
const AdminTransactions: React.FC = () => {
  ...
  const fetchTransactions = async () => {
    setLoading(true);
    try {
      const res = await fetch('https://mondayonsol.fun/crypto-backend/crypto-backend/admin/get_all_transactions.php');
  ...
  const handleAction = async (id: number, type: 'deposit' | 'withdrawal', action: 'approve' | 'decline') => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const res = await fetch(`https://mondayonsol.fun/crypto-backend/crypto-backend/admin/${endpoint}`, {
          method: 'POST',
