...
const MainLayout: React.FC = () => {
  ...
  const fetchNotifications = async () => {
    if (!user.id) return;
    try {
      const res = await fetch(`http://localhost/crypto-backend/get_notifications.php?user_id=${user.id}`);
  ...
  const markNotificationsAsRead = async () => {
    ...
    try {
      await fetch(`http://localhost/crypto-backend/mark_notifications_read.php`, {
        method: 'POST',
